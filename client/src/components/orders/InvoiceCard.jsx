...
const InvoiceCard = memo(({ invoice, onView, onConvert }) => (
    <div className="bg-white rounded-2xl p-6 border border-amber-100 shadow-sm hover:shadow-md transition-all group flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
        <div className={`absolute left-0 top-0 bottom-0 w-1 ${invoice.status === 'converted' ? 'bg-green-500' : 'bg-amber-400'}`}></div>
        ...
        <div className="flex-1 min-w-0 pl-2">
            <div className="flex items-center gap-3 mb-1">
                ...
                {invoice.status === 'converted' && <span className="text-[10px] bg-green-100 text-green-700 px-2 py-0.5 rounded font-bold uppercase tracking-wide">Converted</span>}
            </div>
            ...
            <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-slate-400 text-xs">{invoice.items?.length || invoice.items} items</span>
            </div>
        </div>
        ...
        <div className="w-full md:min-w-[120px] md:w-auto flex md:block justify-between items-center text-right border-t md:border-0 border-gray-100 pt-3 md:pt-0 mt-2 md:mt-0">
            ...
            <span className="text-2xl font-bold text-slate-800">Ksh{(invoice.totals?.total || invoice.totalAmount || 0).toLocaleString()}</span>
        </div>
        ...
        <div className="flex gap-3 min-w-[150px] justify-end">
            {invoice.status !== 'converted' && (
                <button
                    ...
                    className="px-4 py-2.5 rounded-xl bg-green-600 text-white font-bold text-sm hover:bg-green-700 transition-colors shadow-lg shadow-green-200 flex items-center gap-2"
                >
                    <span>✅ Convert</span>
                </button>
            )}
